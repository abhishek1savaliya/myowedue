...
import { Archive, FolderOpen, Globe, Grid2x2, LayoutGrid, List, LoaderCircle, Lock, PlayCircle, Rows3, X } from "lucide-react";
...
function suggestedZipFilename(folderName) {
  const base = String(folderName || "shared-folder")
    .trim()
    .replace(/[^\w.\- ]+/g, "_")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 80);
  return `${base || "shared-folder"}.zip`;
}

const ZIP_DOWNLOAD_MAX_FILES = 40;
const ZIP_DOWNLOAD_MAX_MB = 120;
...
export default function FolderShareClient({ token }) {
  ...
  const allSelected = folder.files.length > 0 && selectedIds.length === folder.files.length;
  ...
  function toggleSelectAll() {
    ...
    if (allSelected) {
      ...
      setSelectedIds(folder.files.map((f) => f.id));
    }
  }
  ...
  async function downloadSelectedZip() {
    ...
    setZipError("");
    try {
      const res = await fetch(`/api/folder-share/${token}/download-zip`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          fileIds: selectedIds,
          bulkAuth: folder.bulkDownloadAuth || undefined,
        }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.message || `ZIP download failed (${res.status}).`);
      }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = suggestedZipFilename(folder.name);
      a.rel = "noopener";
      ...
      setZipError(caught?.message || "Could not download ZIP.");
    } finally {
      setZipLoading(false);
    }
  }
      ...
      <section className="mx-auto max-w-6xl space-y-6">
        ...
        {access?.requiresPassword ? (
          ...
          <>
            <section className="flex flex-col gap-4 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
              <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:gap-6">
                ...
                <p className="text-sm text-zinc-600">
                  {selectedIds.length} of {folder.files.length} selected
                </p>
                <button
                  type="button"
            ...
            {zipError ? <p className="text-sm text-rose-600">{zipError}</p> : null}
            <p className="text-xs text-zinc-500">
              ZIP: up to {ZIP_DOWNLOAD_MAX_FILES} files and {ZIP_DOWNLOAD_MAX_MB} MB combined size per download.
            </p>
            ...
            {folder.files.map((file) => (
              <article key={file.id} className={fileViewClasses.card}>
                ...
                <div className={fileViewClasses.info}>
                  <label className="mb-2 flex cursor-pointer items-center gap-2 text-xs font-medium text-zinc-600">
                    <input
                      type="checkbox"

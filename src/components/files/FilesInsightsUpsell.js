...
import { Folder, HardDrive, Lock, Sparkles } from "lucide-react";
import { uiCard } from "@/lib/ui-classes";
import { cn } from "@/lib/utils";

export default function FilesInsightsUpsell() {
  return (
    <section
      className={cn(
    ...
    >
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm text-zinc-500 dark:text-zinc-400">
              See storage breakdown by type, folder organization, public vs private files, and your largest uploads—Pro
              only.
            </p>
          </div>
        </div>
      ...
      <div className="relative mt-5 grid gap-3 opacity-70 sm:grid-cols-3" aria-hidden>
        {[
          { icon: HardDrive, label: "Storage breakdown" },
          { icon: Folder, label: "Folder stats" },
          { icon: Sparkles, label: "Largest files" },
        ].map(({ icon: Icon, label }) => (
          <div
            key={label}

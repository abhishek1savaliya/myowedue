...
import { Lock, Sparkles, TrendingDown, TrendingUp, Users } from "lucide-react";
import { uiCard } from "@/lib/ui-classes";
import { cn } from "@/lib/utils";

export default function PeopleInsightsUpsell() {
  return (
    <section
      className={cn(
    ...
    >
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold tracking-tight text-foreground sm:text-base">People insights</h2>
            <p className="mt-1 max-w-xl text-sm text-zinc-500 dark:text-zinc-400">
              See total receivables and payables, net exposure, collection rate, and your top contacts at a glance—Pro
              only.
            </p>
          </div>
        </div>
      ...
      <div className="relative mt-5 grid gap-3 opacity-70 sm:grid-cols-3" aria-hidden>
        {[
          { icon: TrendingUp, label: "Top owed to you" },
          { icon: TrendingDown, label: "Top you owe" },
          { icon: Users, label: "Collection rate" },
        ].map(({ icon: Icon, label }) => (
          <div
            key={label}

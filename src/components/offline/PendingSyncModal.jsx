...
export default function PendingSyncModal({ open, items, progress, syncing, onClose, onSyncNow }) {
  ...
  return (
    <div className="fixed inset-0 z-250 flex items-end justify-center bg-black/50 p-2 sm:items-center sm:p-4" role="dialog" aria-modal="true">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-200 bg-white p-3 shadow-xl sm:p-4 dark:border-white/10 dark:bg-zinc-900">
        ...
        <div className="mt-3 max-h-[50vh] space-y-2 overflow-auto pr-1 sm:max-h-72">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item.id} className="rounded-xl border border-zinc-200 p-2.5 text-xs dark:border-white/10">
                <p className="break-all font-medium text-zinc-800 dark:text-zinc-200">
        ...
        <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <button
            type="button"
            onClick={onClose}
            className="min-h-10 rounded-xl border border-zinc-300 bg-white px-3 py-2 text-xs font-medium text-zinc-800 sm:min-h-0 dark:border-white/20 dark:bg-transparent dark:text-zinc-200"
          >
            Close
          </button>
          <button
            ...
            className="min-h-10 rounded-xl border border-emerald-600 bg-emerald-600 px-3 py-2 text-xs font-semibold text-white sm:min-h-0 disabled:opacity-50"
          >
            Sync now
          </button>
        </div>

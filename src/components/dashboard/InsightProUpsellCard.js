...
import { Activity, BarChart3, Lock, Sparkles } from "lucide-react";
import { uiCard } from "@/lib/ui-classes";
import { cn } from "@/lib/utils";

export default function InsightProUpsellCard() {
  return (
    <section
      className={cn(
    ...
    >
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold tracking-tight text-foreground sm:text-base">Advanced Insights</h2>
            <p className="mt-1 max-w-xl text-sm text-zinc-500 dark:text-zinc-400">
              Unlock collection efficiency, open exposure, concentration by person, and quick signals—on top of the
              basic monthly and person charts included in Free.
            </p>
          </div>
        </div>
      ...
      <div className="relative mt-5 grid gap-3 opacity-70 sm:grid-cols-3" aria-hidden>
        {[
          { icon: Activity, label: "Collection efficiency" },
          { icon: BarChart3, label: "Exposure by person" },
          { icon: Sparkles, label: "Concentration signals" },
        ].map(({ icon: Icon, label }) => (
          <div
            key={label}
